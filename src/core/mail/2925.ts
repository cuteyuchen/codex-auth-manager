...
function getProviderMailbox() {
  if (!appConfig["2925EmailAddress"]) {
    throw new Error("2925 邮箱账号未配置，请先在配置页填写 2925EmailAddress");
  }
...
function getProviderLoginPassword() {
  if (!appConfig["2925Password"]) {
    throw new Error("2925 邮箱密码未配置，请先在配置页填写 2925Password");
  }
...
async function fetchLatestEmailWithSession(targetEmail) {
  let session = await get2925Session(false);
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
      const mailList = await fetchMailList({
        mailbox: session.mailbox,
        targetEmail,
        bearerToken: session.bearerToken,
        refreshToken: session.refreshToken,
      ...
      if (latest.messageId) {
        try {
          detail = await fetchMailReadContent({
            mailbox: session.mailbox,
            bearerToken: session.bearerToken,
            refreshToken: session.refreshToken,
            uid: session.uid,
      ...
      const merged = {
        ...latest,
        subject: detail?.subject || latest.subject,
        bodyContent: detail?.bodyContent || latest.bodyContent,
        sender: detail?.sender || latest.sender,
        toAddress: detail?.toAddress?.length ? detail.toAddress : latest.toAddress,
      };
      const matched = findLatestVerificationMail([merged], {
        targetEmail,
        rememberLastCode: false,
        matcher: /(OpenAI|ChatGPT).*(code)|code.*(OpenAI|ChatGPT)/i,
      });
      return {
        id: latest.messageId,
        sender: merged.sender,
        recipient: merged.toAddress,
        subject: merged.subject,
        content: merged.bodyContent,
        html: merged.bodyContent,
        snippet: merged.bodyContent,
        timestamp: latest.createTime,
        receivedAt: latest.createTime ? new Date(latest.createTime).toISOString() : "",
...
export function create2925Provider() {
  return {
    ...
    async getLatestEmail(email) {
      return fetchLatestEmailWithSession(email);
    },
  };
}
