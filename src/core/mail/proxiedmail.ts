...
async function getLatestEmailSummary(email) {
  const binding = await resolveProxyBindingByAddress(email);
  const links = await listReceivedEmailLinks(binding.id);
  const detailed = [];
  for (const item of links) {
  ...
  return {
    id: String(latest.id ?? ""),
    sender: latest.senderEmail,
    recipient: latest.recipientEmail,
    subject: latest.subject,
    content: latest.bodyPlain || latest.bodyHtml,
    html: latest.bodyHtml,
    snippet: latest.bodyPlain || latest.bodyHtml,
