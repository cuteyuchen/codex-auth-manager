...
const session = ref<SessionState>({passwordEnabled: false, authenticated: true});
...
const navGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      {to: "/services", label: "推送服务", icon: Link},
    ],
  },
  {
    label: "系统",
    items: [
      ...
      {to: "/system", label: "系统维护", icon: Tools},
    ],
  },
];
...
async function loadSession() {
  try {
    session.value = await apiGet<SessionState>("/api/session");
  } catch {
    session.value = {passwordEnabled: false, authenticated: true};
  }
}

async function logout() {
  await apiSend("/api/session/logout", "POST");
  await loadSession();
    ...
    <aside class="app-sidebar fixed left-0 top-0 z-50 h-dvh overflow-hidden border-r border-white/10 transition-transform md:sticky md:translate-x-0" :class="{open: mobileMenu}">
      ...
      >
        <template v-for="group in navGroups" :key="group.label">
          <div v-if="!collapsed" class="sidebar-section">{{ group.label }}</div>
    ...
    <section class="flex h-dvh min-w-0 flex-col overflow-hidden">
      <header class="sticky top-0 z-30 flex h-[var(--header-height)] shrink-0 items-center justify-between gap-4 border-b border-[var(--app-border)] bg-[color-mix(in_srgb,var(--app-surface)_92%,transparent)] px-3 backdrop-blur md:px-5">
        ...
        <div class="flex shrink-0 items-center gap-2">
          ...
          <el-button
            v-if="session.passwordEnabled"
            :icon="session.authenticated ? SwitchButton : Lock"
            @click="session.authenticated ? logout() : router.push('/login')"
          >
            {{ session.authenticated ? "退出" : "登录" }}
          </el-button>
        </div>
      </header>
